...
description: 生成 Ian 风格的正文配图，正文可以是任意语言（中文、英文、日文等），图上手写批注默认跟随正文语言。用于用户要求为文章、帖子、博客、Notion 文档、工作流文档、方法论、流程、结构、状态、隐喻或观点生成“怪诞”“小黑”“手绘”“正文配图”“文章插图”“配图建议”“shot list”“去标题/改图”“article illustration”“hand-drawn illustration”等任务；默认使用小黑 IP、纯白手绘、少量红橙蓝批注、简洁清爽但天马行空的视觉风格。
---
...
为文章设计和生成 16:9 横版正文配图。正文可以是任何语言。目标不是做商业插画、PPT 信息图或可爱卡通，而是把文章里的关键判断、流程、结构、状态或隐喻，变成一张清爽、怪诞、有创意、可读但不说明书的手绘解释图。

默认视觉 IP 是“小黑”：黑色实心、白点眼、细腿、空表情，认真做一件荒诞但成立的事。小黑必须参与画面的核心动作，不能只是站在旁边当装饰。

## 标注语言

每个任务先定一个变量：`标注语言`。它只决定图里写什么文字，不改变风格、颜色、结构和小黑的用法。

- 默认 = 正文语言。中文正文写中文，英文正文写英文，日文正文写日文，其他语言同理。
- 用户明确指定时以用户为准，例如“正文是中文，标注用英文”。
- 正文混排多语言时，取正文主体语言。
- 同一张图只用一种文字系统，不要中英日混写。产品名和技术名词（AI、Notion、API、SaaS）保持原样，不翻译。

标注预算按文字系统调整：

- 拉丁字母（英文、法文等）：最多 5-8 处，每处 1-4 个词。
- 非拉丁文字（汉字、假名、韩文、西里尔、阿拉伯文等）：最多 5-6 处，每处 2-8 个字。生图模型在这些文字上更容易写错，宁可少写。
