...
Ian Xiaohei Illustrations 是一个 Codex Skill，用来指导 AI Agent 为文章、帖子、博客、Notion 文档和方法论内容生成正文配图。正文语言不限，中文、英文、日文都可以，图里的手写批注默认跟随正文语言。
...
- 每张图的主题、核心意思、结构类型、小黑动作和标注建议（语言跟随正文）
...
```text
Use $ian-xiaohei-illustrations 为这篇文章设计并生成 5 张小黑怪诞正文配图。
```
...
每张图写清楚：放在哪段后、主题、核心意思、结构类型、小黑在做什么、建议标注词。
...
要求：16:9 横版、纯白背景、黑色手绘线稿、少量红橙蓝手写批注。
...
6. 让小黑承担核心动作
7. 每张图单独调用图像模型生成
8. 按 QA checklist 检查：白底、留白、小黑动作、标注语言与可读性、非 PPT 感、非旧案例复刻
9. 保存最终 PNG，并报告用途和路径
...
- 图片里的文字越短越稳定；非拉丁文字（中文、日文、韩文等）比英文更容易写错，标注要更少更短。
- 默认标注语言跟随正文；需要英文配图时，直接在 prompt 里写明“标注用英文”。
- 每张图只讲一个核心结构，不要把文章做成说明书。
- 小黑必须承担核心动作；如果去掉小黑画面仍然完全成立，说明小黑太装饰了。
- 示例图只用于校准线条密度、留白、颜色克制和小黑参与方式，不要复刻构图。
- AI 图像模型可能出现错字、幻觉标签、风格漂移或多余标题，生成后需要检查。
- 如果错字严重，优先减少标注词并重生成；如果语言写错或混写，把标注语言在 prompt 里写死成一种。
